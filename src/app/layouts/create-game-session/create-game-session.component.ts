...
import { FormBuilder, FormGroup, ReactiveFormsModule, Validators, AbstractControl, ValidationErrors, NonNullableFormBuilder, FormControl } from '@angular/forms';
      ...
      <div class="w-full max-w-md border-2 border-gray-700 p-6 rounded-lg">
        <form [formGroup]="sessionForm" (ngSubmit)="onSubmit()" class="w-full">
          <div class="mb-6">
            ...
            <p class="text-xs text-gray-400 mt-1">This is not the title of the game. It's alias, what can see only you. It's will help to continue or start the game late. You can finde it in the started games list.</p>
          </div>
          ...
          <div class="mb-6 relative mt-8">
            ...
            <input 
              ...
              min="0">
            <div class="absolute -bottom-5 left-0">
              @if (sessionForm.get('aiPlayerCount')?.errors?.['required'] && sessionForm.get('aiPlayerCount')?.touched) {
              ...
              @if (sessionForm.get('aiPlayerCount')?.errors?.['exceedsPlayerCount'] && sessionForm.get('aiPlayerCount')?.touched) {
...
export class CreateGameSessionComponent implements OnInit {
  ...
  public ngOnInit(): void { 
    ...
    this.sessionForm.controls.playerCount.valueChanges
  ...
  public onSubmit(): void {
    if (this.sessionForm.valid) {
      ...
      this.gameSessionService.createGameSession(gameSessionParams)
  ...
  private initializeForm(): void {
    this.sessionForm = this.formBuilder.group({
      ...
      playerCount: this.formBuilder.control(3, { validators: [Validators.required, Validators.min(2), Validators.max(10)] }),
      aiPlayerCount: this.formBuilder.control(0, { validators: [Validators.required, Validators.min(0), this.aiPlayerCountValidator()] }),
